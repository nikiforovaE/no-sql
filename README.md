...
Для успешного запуска проекта локально потребуются:

1. Установленный [Docker](https://docs.docker.com/get-docker/) и Docker Compose daemon.
2. Утилита `make` (для выполнения команд автоматизации запуска).
...
1. **Запуск всех сервисов:**
   ...
   make run
   ```
   *Команда автоматически соберет Docker-образ Java-приложения, развернет контейнеры баз данных (Redis, кластер MongoDB,
   Cassandra, Neo4j), запустит скрипты их инициализации и само приложение.*

2. **Проверка доступности:**
   После завершения инициализации баз данных проверить статус приложения:
   ```bash
...
3. **Остановка проекта:**
   Для остановки приложения и удаления всех запущенных Docker-контейнеров выполнить команду:
...
Управление конфигурацией приложения и Docker-контейнеров осуществляется через
единый [файл переменных окружения](.env.local).

### Таблица переменных окружения (.env.local)

| Переменная                | Описание                                             | Значение по умолчанию |
|:--------------------------|:-----------------------------------------------------|:----------------------|
| `APP_HOST`                | Хост, на котором запускается веб-приложение          | `0.0.0.0`             |
| `APP_PORT`                | Порт встроенного веб-сервера                         | `8080`                |
| `APP_USER_SESSION_TTL`    | Время жизни сессии пользователя в Redis (в секундах) | `60`                  |
| `APP_LIKE_TTL`            | Время жизни кэша лайков в Redis (в секундах)         | `60`                  |
| `APP_EVENT_REVIEWS_TTL`   | Время жизни кэша отзывов в Redis (в секундах)        | `120`                 |
| `APP_RECOMMENDATIONS_TTL` | Время жизни кэша рекомендаций в Redis (в секундах)   | `60`                  |
| `REDIS_HOST`              | Хост СУБД Redis                                      | `redis`               |
| `REDIS_PORT`              | Сетевой порт Redis                                   | `6379`                |
| `REDIS_PASSWORD`          | Пароль для подключения к Redis                       | *(пусто)*             |
| `REDIS_DB`                | Номер используемой базы данных Redis                 | `0`                   |
| `MONGODB_DATABASE`        | Название базы данных MongoDB                         | `eventhub`            |
| `MONGODB_USER`            | Имя пользователя для подключения к MongoDB           | `eventhub`            |
| `MONGODB_PASSWORD`        | Пароль для подключения к MongoDB                     | `eventhub`            |
| `MONGODB_HOST`            | Хост маршрутизатора MongoDB (mongos)                 | `mongos`              |
| `MONGODB_PORT`            | Сетевой порт роутера MongoDB                         | `27017`               |
| `MONGO_SHARD_PORT`        | Сетевой порт шардов MongoDB                          | `27018`               |
| `CASSANDRA_HOSTS`         | Хост СУБД Apache Cassandra                           | `cassandra-test`      |
| `CASSANDRA_PORT`          | Сетевой порт Cassandra                               | `9042`                |
| `CASSANDRA_KEYSPACE`      | Название ключевого пространства Cassandra            | `testkeyspace`        |
| `CASSANDRA_CONSISTENCY`   | Уровень согласованности запросов в Cassandra         | `ONE`                 |
| `NEO4J_HOST`              | Имя контейнера / хост Neo4j                          | `neo4j`               |
| `NEO4J_URL`               | Строка подключения к Neo4j по протоколу Bolt         | `bolt://neo4j:7687`   |
| `NEO4J_USERNAME`          | Логин администратора Neo4j                           | `neo4j`               |
| `NEO4J_PASSWORD`          | Пароль администратора Neo4j                          | `password`            |
| `NEO4J_HTTP_PORT`         | Порт веб-панели управления Neo4j (HTTP)              | `7474`                |
| `NEO4J_BOLT_PORT`         | Сетевой порт Neo4j для драйверов (Bolt)              | `7687`                |

---

## 🧪 Тестирование

Проект полностью покрыт автоматическими тестами, проверяющими соответствие бизнес-требованиям.

* **Автоматические тесты:** Запуск интеграционных проверок и тестирование API-эндпоинтов выполняются автоматически в
  облачном окружении **GitHub Actions** при создании Pull Request или пуше в ветку решения.
* **Конфигурация проверок:** Пайплайн считывает текущий номер лабораторной работы из файла `.labrc` и запускает
  соответствующий набор проверок.
* Узнать подробности о структуре и логике тестов можно в
  [репозитории курса ndbx](https://github.com/sitnikovik/ndbx).
